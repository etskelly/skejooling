<template>
    <div>
        <div class="grid-scroller">
            ...
            <div ref="schedulegrid" class="grid border-solid border-2 border-slate-700" :style="{ gridTemplateColumns: '200px repeat(' + timePeriodValues.length + ', 1fr)' }">
                <div class="vl" :style="{left: calculateVerticalLinePosition}"></div>
                <!-- <div class="vl" style="left: calc(200px + 50%)"></div> -->
                <div class="grid-header-span text-black text-center font-bold" :style="{ gridColumn: '1 / -1' }" style="min-height: 40px;">
   {{ monthName }}
</div>
                <div class="grid-header fixed-left">Name</div>
                <div v-for="n, idx in timePeriodValues" class="grid-header"> {{ n.format(timePeriodFormat) }}</div>
